...
Future<void> main() async {
  ...
  // Initialize logger
  LoggerService.init();
  LoggerService.info('Starting KPFK Radio app');
  ...
  try {
    ...
    await AudioService.init(
      builder: () => getIt<KPFKAudioHandler>(),
    ...
    if (Platform.isIOS) {
      try {
        final audioHandler = getIt<KPFKAudioHandler>();
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) async {
    if (state == AppLifecycleState.detached) {
      try {
        ...
        final handler = getIt<KPFKAudioHandler>();
...
class KPFKRadioApp extends StatelessWidget {
  const KPFKRadioApp({super.key});
