...
class DebugOutagePreset {
  ...
  static const live = DebugOutagePreset(
    ...
    description: 'The real KPFK stream.',
    url: null,
  ...
  static const timeout = DebugOutagePreset(
    ...
    url: 'http://10.255.255.1/kpfk.m3u',
    expected: 'OUTAGE modal after the watchdog fires.',
  );

  static const notFound = DebugOutagePreset(
    label: 'Playlist 404',
    description: 'The .m3u is gone from the docs host.',
    url: 'https://docs.pacifica.org/kpfk/this-playlist-does-not-exist.m3u',
    expected: 'OUTAGE modal.',
  );

  /// Resolves fine but the body is HTML, so M3UParser finds no stream URL —
  /// the exact shape of the dead-playlist outage that prompted this work.
  static const deadPlaylist = DebugOutagePreset(
    label: 'Playlist has no stream',
    description: 'URL loads but contains no stream (returns a web page).',
    url: 'https://docs.pacifica.org/',
    expected: 'OUTAGE modal — "no stream URL in playlist".',
  );
