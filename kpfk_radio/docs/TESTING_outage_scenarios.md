...
and edit that constant to `http://<your-mac-lan-ip>:8000/kpfk.m3u`.
...
| Dead playlist | `echo "garbage" > kpfk.m3u` — expect **outage** modal |
| 503 / 500 | serve via a tiny handler returning that status — expect **outage** modal |

The device and Mac must be on the same network, and iOS needs the plain-HTTP
exception that `NSAllowsArbitraryLoads` already provides.
