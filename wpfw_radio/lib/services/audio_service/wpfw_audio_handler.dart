...
class WPFWAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  void _setInitialMediaItem() {
    _currentMediaItem = MediaItem(
      ...
      album: "Live Radio",
      title: "WPFW 89.3 FM",
      artist: "Jazz and Justice Radio",
      duration: const Duration(hours: 24),
      artUri: Uri.parse("https://confessor.wpfwfm.org/playlist/images/wpfw_logo.png"),
    );
    ...
    LoggerService.info('🔍 INITIAL LOAD FIX: _setInitialMediaItem() called but NOT showing generic player');
    LoggerService.info('🎯 INITIAL LOAD FIX: Waiting for real metadata before showing player');
  }



  static Future<WPFWAudioHandler> create() async {
    final player = AudioPlayer();

    LoggerService.info(
        '🎵 Initializing audio handler (single source of truth)');

    return WPFWAudioHandler._(
      player,
  ...
  Future<void> _init() async {
    try {
      LoggerService.info('🎵 AudioHandler: Initializing with EXPERT M3U parsing');
      ...
      LoggerService.info('🎯 SAMSUNG FIX: Audio session configured and activated for lockscreen controls');
      ...
      if (Platform.isAndroid) {
        mediaItem.listen((item) {
          final t = item?.title ?? '';
          final a = item?.artist ?? '';
          LoggerService.info('🤖 ANDROID DIAG: mediaItem changed -> title="$t" artist="$a"');
        });
        playbackState.listen((state) {
          // throttle
          final now = DateTime.now();
          if (_lastAndroidDiag == null || now.difference(_lastAndroidDiag!) > const Duration(seconds: 2)) {
  ...
  void _broadcastState([PlaybackEvent? event]) {
    ...
    LoggerService.info('🎯 ONE TRUTH: _broadcastState called - MediaItem=${_currentMediaItem?.title}, playing=${_player.playing}, state=${_player.processingState}');
    LoggerService.info('🎯 ONE TRUTH: This is the SINGLE SOURCE for all mediaItem.add() calls');
  }
    ...
    switch (state) {
      ...
      case ProcessingState.buffering:
        ...
        if (_lastBufferingUpdate == null ||
            now.difference(_lastBufferingUpdate!) > const Duration(seconds: 5)) {
  ...
  Future<void> play() async {
    try {
      ...
      if (!success) {
        LoggerService.error('🎯 SAMSUNG FIX: Failed to gain audio focus - lockscreen controls may not work');
        return;
      }
      LoggerService.info('🎯 SAMSUNG FIX: Audio focus gained successfully - lockscreen controls should now work');
      ...
      LoggerService.info('🎯 ONE TRUTH: Calling _player.play() - event listener will trigger _broadcastState');
      await _player.play();
      ...
      if (_currentMetadata != null) {
        LoggerService.info('🔍 METADATA DELAY FIX: Using existing metadata for Samsung notification');
        await SamsungMediaSessionService.updateMetadata(
          _currentMetadata!.currentSong,
          _currentMetadata!.artist,
        );
      } else {
        LoggerService.info('🔍 METADATA DELAY FIX: No metadata available yet - Samsung will show static until metadata arrives');
      }
      
      // STANDARD BEHAVIOR: Show notification only when PLAYING starts
      await SamsungMediaSessionService.showNotification();
      LoggerService.info('🔍 SAMSUNG DEBUG: Notification shown because PLAY was pressed (STANDARD)');
  ...
  Future<void> pause() async {
    try {
      ...
      LoggerService.info('🔍 SAMSUNG DEBUG: Notification hidden because PAUSE was pressed (STANDARD)');
  ...
  Future<void> stop() async {
    try {
      ...
      await SamsungMediaSessionService.hideNotification();
      LoggerService.info('🔍 SAMSUNG DEBUG: Notification hidden because STOP was pressed');
      ...
      mediaItem.add(null);
      LoggerService.info('🎯 STOP: Player removed from notification tray - MediaItem set to null');
      ...
      LoggerService.info('🎯 STOP: Set playback state to idle with no controls to remove service');
  ...
  Future<void> seek(Duration position) async {
    ...
    LoggerService.info('🎵 AudioHandler: Seek requested but not supported for live streams');
  }
  ...
  Future<void> _updateMediaSession(bool playing, MediaItem mediaItem) async {
    ...
    LoggerService.info('🎵 AudioHandler: Updated playback state only, not metadata');
  }
  ...
  Future<void> _updateMediaItem(String title, String artist) async {
    ...
    LoggerService.info('🔍 METADATA BATTLE: _updateMediaItem() called with REAL metadata: "$title" by "$artist"');
    LoggerService.info('🎯 ONE TRUTH: Updated _currentMediaItem with real metadata: "$title" by "$artist"');
    LoggerService.info('🎯 ONE TRUTH: Next _broadcastState call will use this updated MediaItem');
    
    // CRITICAL: Update Samsung MediaSession with real metadata
    LoggerService.info('🔍 SAMSUNG DEBUG: Calling SamsungMediaSessionService.updateMetadata("$title", "$artist")');
    await SamsungMediaSessionService.updateMetadata(title, artist);
    LoggerService.info('🔍 SAMSUNG DEBUG: SamsungMediaSessionService.updateMetadata() completed');
  }
  ...
  Future<void> _updatePlaybackStateOnly() async {
    ...
    LoggerService.info('🎵 AudioHandler: Playback state updated, using Swift for lockscreen metadata');
  }
  ...
  Future<void> forceReinitialize() async {
    try {
      LoggerService.info('🎵 AudioHandler: FORCE REINITIALIZE - Complete reset');
      ...
      LoggerService.info('🎵 AudioHandler: Force reinitialize complete - ready for playback');
    } catch (e) {
  ...
  Future<String> _resolveStreamUrl(String url) async {
    try {
      ...
      LoggerService.info('🎵 AudioHandler: Extracted direct stream URL: $directUrl');
      return directUrl;
  ...
  Future<void> updateMediaItem(MediaItem mediaItem) async {
    LoggerService.info('🔍 METADATA BATTLE: updateMediaItem() called with title="${mediaItem.title}", artist="${mediaItem.artist}"');
    LoggerService.info('🔍 METADATA BATTLE: This is a COMPETING SOURCE - should be eliminated!');
    ...
    if (Platform.isAndroid) {
      LoggerService.info('🎯 METADATA FIX: Updating _currentMediaItem instead of direct mediaItem.add()');
      _currentMediaItem = mediaItem;
