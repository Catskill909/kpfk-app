...
class _HomePageState extends State<HomePage> {
  ...
  void _startSpinnerTimeout() {
    ...
    _spinnerTimeoutTimer = Timer(_maxSpinnerDuration, () {
      if (_showLocalLoading && mounted) {
        LoggerService.warning('🔄 SPINNER TIMEOUT: Force reset loading state after ${_maxSpinnerDuration.inSeconds}s');
        setState(() {
  ...
  bool _isMediumTablet(BuildContext context) {
    final size = MediaQuery.of(context).size;
    return size.shortestSide > 600 && size.shortestSide <= 800; // Regular tablets
  }
  ...
  Widget build(BuildContext context) {
    final isOnline = context.select<ConnectivityCubit, bool>((c) => c.state.isOnline);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: Builder(
          builder: (context) => IconButton(
            icon: Icon(
              Icons.menu,
              size: _isLargeTablet(context) ? 48 : (_isMediumTablet(context) ? 38 : (_isSmallPhone(context) ? 26 : 30)),
              color: Colors.white,
            ),
        ...
        title: Image.asset(
          ...
          height: _isLargeTablet(context) ? 70 : (_isMediumTablet(context) ? 60 : (_isSmallPhone(context) ? 34 : 40)),
          fit: BoxFit.contain,
        ),
        centerTitle: true,
        actions: [
      ...
      body: SafeArea(
        ...
        listener: (context, state) {
          ...
          LoggerService.debug('🔄 SPINNER: State changed to ${state.playbackState}, _showLocalLoading: $_showLocalLoading');
          ...
          if (_showLocalLoading && (state.playbackState == StreamState.playing || state.playbackState == StreamState.error)) {
            LoggerService.debug('🔄 SPINNER: Clearing spinner - state is ${state.playbackState}');
            setState(() {
              _showLocalLoading = false;
            });
            _cancelSpinnerTimeout();
          } else if (_showLocalLoading) {
            LoggerService.debug('🔄 SPINNER: Keeping spinner - state is ${state.playbackState}');
          }
          ...
          if (_userPressedPause && (state.playbackState == StreamState.paused || state.playbackState == StreamState.initial)) {
            setState(() {
          ...
          if (_lastAnnouncedPlayback != state.playbackState) {
            ...
            switch (state.playbackState) {
              case StreamState.playing:
                SemanticsService.announce('Playing WPFW stream', dir);
                break;
          ...
          if (currentShow != null && currentShow.isNotEmpty && currentShow != _lastAnnouncedShow) {
          ...
          if (state.errorMessage != null) {
            ...
            SemanticsService.announce(state.errorMessage!, Directionality.of(context));
          }
        },
            ...
            children: [
              ...
              Positioned.fill(
                child: SingleChildScrollView(
                  padding: EdgeInsets.only(bottom: 100), // Add padding for bottom buttons
                  child: Column(
                    children: [
                  Container(
                width: MediaQuery.of(context).size.width,
                padding: EdgeInsets.symmetric(horizontal: _isSmallPhone(context) ? 12.0 : 16.0),
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    // Station Logo
                    Container(
                      width: MediaQuery.of(context).size.width * (_isSmallPhone(context) ? 0.8 : (MediaQuery.of(context).size.shortestSide > 600 ? 0.7 : 0.85)),
                      height: MediaQuery.of(context).size.width * (_isSmallPhone(context) ? 0.8 : (MediaQuery.of(context).size.shortestSide > 600 ? 0.7 : 0.85)),
                      margin: EdgeInsets.only(top: _isSmallPhone(context) ? 12 : 20),
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(20),
                        border: Border.all(
                          color: const Color(0x1AFFFFFF), // ~10% white
                          width: MediaQuery.of(context).size.shortestSide > 600 ? 1 : 2,
                        ),
                        boxShadow: [
                          BoxShadow(
                            color: Colors.black.withValues(red: 0, green: 0, blue: 0, alpha: 77), // ~0.3 opacity
                            blurRadius: 8,
                      ...
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(20),
                        child: state.metadata?.current.hasHostImage == true
                            ? Image.network(
                    ...
                    if (state.metadata != null) ...[
                      SizedBox(height: _isSmallPhone(context) ? 16 : 20),
                      Text(
                        state.metadata!.current.showName,
                        style: AppTextStyles.showTitleForDevice(MediaQuery.of(context).size),
                        textAlign: TextAlign.center,
                        maxLines: 2,
                      ...
                      Text(
                        ...
                        style: AppTextStyles.showTimeForDevice(MediaQuery.of(context).size),
                        textAlign: TextAlign.center,
                        maxLines: 2,
                        overflow: TextOverflow.ellipsis,
                      ),
                      if (state.metadata!.current.hasSongInfo) ...[
                        SizedBox(height: _isSmallPhone(context) ? 8 : 10),
                        Text(
                          'Song: ${state.metadata!.current.songTitle} - ${state.metadata!.current.songArtist}',
                          style: AppTextStyles.bodyLargeForDevice(MediaQuery.of(context).size),
                          textAlign: TextAlign.center,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
                        ...
                        Text(
                          ...
                          style: AppTextStyles.bodyMediumForDevice(MediaQuery.of(context).size),
                          textAlign: TextAlign.center,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
              ...
              Container(
                ...
                margin: EdgeInsets.symmetric(vertical: _isSmallPhone(context) ? 24.0 : 32.0),
                child: Stack(
                  alignment: Alignment.center,
                  children: [
                    Semantics(
                      button: true,
                      enabled: true,
                      label: state.playbackState == StreamState.playing
                          ? 'Stop stream and reset'
                          : 'Play stream',
                      hint: 'Double tap to ${state.playbackState == StreamState.playing ? 'stop and reset' : 'play'}',
                      child: IconButton(
                        iconSize: _isSmallPhone(context) ? 90.0 : (MediaQuery.of(context).size.shortestSide > 600 ? 150.0 : 120.0),
                        ...
                        onPressed: (!isOnline ||
                                state.playbackState == StreamState.loading ||
                            ...
                            : () {
                                if (state.playbackState == StreamState.playing) {
                                  ...
                                  context.read<StreamBloc>().add(PauseStream());
                                } else {
                                  // PLAY: Show spinner - starting stream takes time
                                  LoggerService.debug('🔄 SPINNER: Play button pressed, current state: ${state.playbackState}');
                                  setState(() {
                                    _showLocalLoading = true;
                                    _userPressedPause = false;
                                  });
                                  LoggerService.debug('🔄 SPINNER: Spinner enabled, starting timeout');
                      ...
                      Positioned.fill(
                        child: Center(
                          child: Semantics(
                            ...
                            liveRegion: true,
                            child: SizedBox(
                              width: _isSmallPhone(context) ? 110.0 : 140.0,
                              height: _isSmallPhone(context) ? 110.0 : 140.0,
                ...
                child: SafeArea(
                  ...
                  child: Semantics(
                    ...
                    child: RawMaterialButton(
                      ...
                      elevation: 6,
                      fillColor: const Color(0xFF1E1E1E), // dark gray chip background
                      shape: const CircleBorder(
                        side: BorderSide(color: Color(0x1AFFFFFF), width: 1), // subtle 10% white border
                      ),
                      constraints: BoxConstraints.tightFor(
                        width: _isSmallPhone(context) ? 48 : (_isLargeTablet(context) ? 72 : 56),
                        height: _isSmallPhone(context) ? 48 : (_isLargeTablet(context) ? 72 : 56),
                      ),
                      child: Icon(
                        Icons.volunteer_activism,
                        color: Colors.white,
                        size: _isSmallPhone(context) ? 20 : (_isLargeTablet(context) ? 32 : 24),
                      ),
                    ),
                  ),
                ),
                  ...
                  child: Semantics(
                    ...
                    child: RawMaterialButton(
                      ...
                      child: BlocBuilder<SleepTimerCubit, SleepTimerState>(
                        ...
                        builder: (context, state) {
                          if (state is SleepTimerRunning || state is SleepTimerPaused) {
                            ...
                            return Column(
                              ...
                              children: [
                                Icon(
                                  Icons.timer,
                                  color: Colors.white,
                                  size: _isSmallPhone(context) ? 16 : (_isLargeTablet(context) ? 24 : 18),
                                ),
                                ...
                                Text(
                                  ...
                                  style: TextStyle(
                                    color: Colors.white,
                                    fontSize: _isSmallPhone(context) ? 9 : (_isLargeTablet(context) ? 14 : 11),
