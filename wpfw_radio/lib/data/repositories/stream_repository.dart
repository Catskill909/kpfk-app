...
class StreamRepository {
  final WPFWAudioHandler _audioHandler;
  ...
  StreamRepository({
    required WPFWAudioHandler audioHandler,
  ...
  Future<void> stopAndColdReset({bool preserveMetadata = false}) async {
    try {
      LoggerService.info('🎵 StreamRepository: stopAndColdReset started (preserveMetadata: $preserveMetadata)');
      
      // Store current metadata before any operations if preserving
      StreamMetadata? savedMetadata;
      if (preserveMetadata) {
        savedMetadata = _currentMetadata;
        LoggerService.info('🎵 StreamRepository: Preserving current metadata: ${savedMetadata?.current.showName}');
      }
      ...
      if (!preserveMetadata) {
        ...
        try {
          ...
          LoggerService.info('🎵 StreamRepository: Lockscreen cleared (full reset)');
        } catch (_) {}
      } else {
        LoggerService.info('🎵 StreamRepository: Skipping lockscreen clear to preserve metadata');
      }
      ...
      if (!preserveMetadata) {
        ...
        LoggerService.info('🎵 StreamRepository: Metadata preserved and restored');
      ...
      LoggerService.info('🎵 StreamRepository: stopAndColdReset completed (preserveMetadata: $preserveMetadata)');
    } catch (e) {
    ...
    _playbackStateSubscription = _audioHandler.playbackState.listen(
      (playbackState) {
        ...
        if (Platform.isIOS && _currentMetadata != null) {
          LoggerService.info(
              '🎵 Playback state changed: playing=$isPlaying');
  ...
  Future<void> play({AudioCommandSource? source}) async {
    try {
      LoggerService.info('🎵 StreamRepository: Play requested from ${source ?? 'UI'} - checking server health first');
      ...
      try {
        ...
        if (!healthResult.isHealthy) {
          LoggerService.info('🎵 StreamRepository: Server health check failed: ${healthResult.errorType}');
          await _handleServerError(healthResult);
          return;
        }
        
        LoggerService.info('🎵 StreamRepository: Server health check passed - proceeding with playback');
        
      } on NetworkConnectivityException catch (e) {
        LoggerService.info('🎵 StreamRepository: Network connectivity issue during health check: $e');
      ...
      if (source == AudioCommandSource.lockscreen) {
        LoggerService.info('🎵 StreamRepository: Lockscreen-initiated playback detected');
  ...
  void restartMetadataService() {
    LoggerService.info('🎵 StreamRepository: Restarting metadata service after network recovery');
    _metadataService.startFetching();
  ...
  void _updateMediaMetadata(StreamMetadata metadata) {
    ...
    final String title = showInfo.showName.isNotEmpty
        ? showInfo.showName
        : 'WPFW Radio'; // Fallback only if empty
    ...
    if (showInfo.hasSongInfo &&
      ...
      // Just show host name
      artist =
          showInfo.host.isNotEmpty ? 'Host: ${showInfo.host}' : 'WPFW 89.3 FM';
    }
    ...
    if (Platform.isAndroid && !isPlaying) {
      LoggerService.info('🤖 ANDROID: Skipping updateMediaItem because not playing yet');
      
      // CRITICAL: But still update Samsung MediaSession even when not playing
      LoggerService.info('🔍 SAMSUNG DEBUG: Updating Samsung MediaSession directly (not playing)');
      try {
        // Import needed at top of file
        // await SamsungMediaSessionService.updateMetadata(title, artist);
        LoggerService.info('🔍 SAMSUNG DEBUG: Samsung MediaSession updated with metadata while not playing');
      } catch (e) {
        LoggerService.error('🔍 SAMSUNG DEBUG: Failed to update Samsung MediaSession: $e');
      }
    } else {
      LoggerService.info('🎵 Handler.updateMediaItem: platform=${Platform.isAndroid ? 'Android' : 'iOS'}, isPlaying=$isPlaying');
  ...
  Future<void> _handleServerError(AudioServerHealthResult healthResult) async {
    LoggerService.info('🎵 StreamRepository: Handling server error: ${healthResult.errorType}');
  ...
  Future<void> _resetAudioControlsForServerError() async {
    try {
      LoggerService.info('🎵 StreamRepository: Resetting audio controls for server error');
  ...
  void clearServerError() {
    ...
    AudioServerHealthChecker.clearCache(); // Clear health check cache for fresh retry
    _updateState(StreamState.initial);
  }
  ...
  Future<void> forceAudioReinitialize() async {
    try {
      LoggerService.info('🎵 StreamRepository: FORCE AUDIO REINITIALIZE - Complete system reset');
      ...
      LoggerService.info('🎵 StreamRepository: Force audio reinitialize complete');
    } catch (e) {
