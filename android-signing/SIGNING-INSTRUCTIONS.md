...
## What You Need to Build (Handoff Checklist)

A future developer needs **two things** to build and ship this app:

1. **The GitHub source repository** (all code, project config, and dependency lists).
2. **This `android-signing/` folder** — containing `kpfk-release.jks`, `key.properties`,
   and this file.

> ⚠️ **Important:** The keystore (`*.jks`) and `key.properties` are **gitignored** — they are
> NOT in the GitHub repo. They must be delivered separately (which is how you received this
> folder). Without this folder you cannot sign a release the Play Store will accept.

Everything else (`local.properties`, iOS `Generated.xcconfig`, `Pods/`, build directories) is
auto-regenerated by Flutter and does **not** need to be handed off.

**Full setup, start to finish:**

```bash
# 1. Clone the repo (folder must be named kpfk-app — see storeFile note below)
git clone <repo-url> kpfk-app
cd kpfk-app

# 2. Place this android-signing/ folder at the repo root (if not already there)

# 3. Copy signing credentials into the Flutter Android project (gitignored target)
cp android-signing/key.properties kpfk_radio/android/key.properties

# 4. Install dependencies and build the signed App Bundle
cd kpfk_radio
flutter pub get
...
Requires the Flutter SDK installed (built and verified on Flutter 3.44.3).

**Questions / to obtain the keystore:** contact Paul H (paullnyc@gmail.com) or gm@kpfk.org.
